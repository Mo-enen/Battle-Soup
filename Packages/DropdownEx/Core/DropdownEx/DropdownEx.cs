...
namespace UnityEngine.UI {
	...
	public class DropdownEx : Selectable, IPointerClickHandler, ISubmitHandler, ICancelHandler {
		...
		public string MultipleSelectedText = "(Mix)";
		public string SingleSelectedText = "{0}";
		public string AllSelectedText = "All";
		...
		protected virtual void updateOptionsState (uint added, uint removed, bool invokeEvent = true) {
			uint index = 0;
			while (added > 0) {
				if ((added & 0x01) == 0x01) {
			...
			if (0 == options.Count) {
				...
				if (m_CaptionText != null) {
					if (0 == itemCount) {
						...
						string selectedName = !string.IsNullOrEmpty(data.text) ? data.text : "";
						m_CaptionText.text = string.Format(SingleSelectedText, selectedName);
					} else if (options.Count == itemCount) {
						m_CaptionText.text = !string.IsNullOrEmpty(AllSelectedText) ? AllSelectedText : "";
					} else {
		...
		public void AddOptions (IEnumerable<OptionData> options) {
			...
			foreach (var op in options) {
				list.Add(new OptionData(op.text, op.image, op.selected));
			}
			this.m_Options.AddRange(list);
			RefreshShownValue();
		}
		...
		public void SetValueWithoutNotify (uint value) {
			...
			if (AllowMultiSelect) {
				//uint added_mask = ~m_Value & value;
				//uint removed_mask = m_Value & ~value;
				//updateOptionsState(added_mask, removed_mask, false);

				for (int i = 0; i < options.Count; i++) {
					options[i].selected = (value >> i) % 2 == 1;
				}
