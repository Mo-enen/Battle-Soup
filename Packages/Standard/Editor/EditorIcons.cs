			...
			using (new GUILayout.HorizontalScope(EditorStyles.helpBox, GUILayout.MaxHeight(viewBigIcons ? 140 : 120))) {
				...
				using (new GUILayout.VerticalScope()) {
					...
					if (GUILayout.Button("Export", EditorStyles.miniButton)) {
						var texture = new Texture2D(iconSelected.image.width, iconSelected.image.height) {
							alphaIsTransparency = true
						};
						Graphics.CopyTexture(iconSelected.image, texture);
						Util.ByteToFile(texture.EncodeToPNG(), $"Assets/{iconSelected.tooltip}.png");
